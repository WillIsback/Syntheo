...
test("PUT audio retourne 401 si non authentifié", async (t) => {
  const authMock = mock.method(audioRouteDeps, "auth", async () => null);
  t.after(() => authMock.mock.restore());

  const res = await PUT(makeRequest(SESSION_ID), makeParams(SESSION_ID));
  assert.equal(res.status, 401);
});

test("PUT audio retourne 400 pour un sessionId non-UUID", async (t) => {
  const authMock = mock.method(audioRouteDeps, "auth", async () => ({
    user: { id: USER_ID },
  }));
  t.after(() => authMock.mock.restore());

  const res = await PUT(makeRequest("not-a-uuid"), makeParams("not-a-uuid"));
  assert.equal(res.status, 400);
});

test("PUT audio retourne 404 si la session n'appartient pas à l'utilisateur", async (t) => {
  const authMock = mock.method(audioRouteDeps, "auth", async () => ({
    user: { id: USER_ID },
  }));
...
test("PUT audio streame vers WhisperX, attache le job_id et retourne 200", async (t) => {
  const authMock = mock.method(audioRouteDeps, "auth", async () => ({
    user: { id: USER_ID },
  }));
  const getSessionMock = mock.method(
...
test("PUT audio marque la session échouée et relance l'erreur si WhisperX échoue", async (t) => {
  const authMock = mock.method(audioRouteDeps, "auth", async () => ({
    user: { id: USER_ID },
  }));
  const getSessionMock = mock.method(
  ...
  await assert.rejects(
    () => PUT(makeRequest(SESSION_ID), makeParams(SESSION_ID)),
